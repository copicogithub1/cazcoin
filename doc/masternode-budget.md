...
CazCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<cazcoin_address\> \<monthly_payment_cazcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ CazCoin, _12_ times over the course of a year totaling _24000_ CazCoin.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<cazcoin_address\> \<monthly_payment_cazcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` CazCoin.
 ...
 * vote-many          - Vote on a CazCoin initiative
 * vote-alias         - Vote on a CazCoin initiative
 * vote               - Vote on a CazCoin initiative/budget
